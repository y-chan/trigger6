...
cargo run --release --features usb --bin t6-virtual-display -- \
  ...
  --type7-large-tile-fps-divisor 2 \
  --report-every 60 \
  --profile
```
...
- `--no-frame-throttle` は小-中サイズ tile では fps を上げることがあるが、large tile を 30 fps 相当に落とす制御と併用しづらい。
- large tile の fps 制御を使う場合は `--no-frame-throttle` を外す。
...
- JPEG は baseline 4:2:0 を前提にし、component id を `0,1,2` へ寄せる。
