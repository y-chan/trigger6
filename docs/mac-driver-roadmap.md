...
- macOS 側は、当面 `CGVirtualDisplay` + `CGDisplayStream` + userspace `rusb` の通常アプリ構成で進める。
- kext / DriverKit は、別ベンダドライバを削除して `rusb` の interface claim が通る前提では必須ではない。
- 常駐、デバイス検出、回転指定、転送モード切替、診断表示は Tauri のメニューバーアプリとして実装する。
...
目的は、実験用 CLI をそのまま使う段階から、日常的に起動できる macOS アプリへ移すこと。

基本方針:

- アプリは通常の `.app` として作る。
- USB 転送は既存の `t6proto` / `rusb` path を使う。
- macOS の表示面は `CGVirtualDisplay` で作り、`CGDisplayStream` から得た frame を T6 へ送る。
- USB device を OS の display driver として登録しない。
- kext、DriverKit、System Extension は初期実装に含めない。

メニューバー UI:

- 接続状態を表示する。
- `0711:5601` の検出、接続、切断、再接続を扱う。
- 表示の開始、停止を切り替える。
- 回転を `0`, `90`, `180`, `270` から選ぶ。
- 解像度、fps、品質、transport を選ぶ。
- `--reset-jpeg-engine` 相当の復旧操作を出す。
- 診断ログと直近の profile 値を見られるようにする。

起動と常駐:

- 最初は明示起動のメニューバーアプリにする。
- 自動起動は Tauri autostart plugin か LaunchAgent を使う。
- device hotplug は、まず polling で実装する。
- polling が UI 体験や消費電力の問題になった場合だけ、IOKit notification へ移す。
- sleep / wake 後は USB handle、仮想ディスプレイ、sender thread を作り直す。

競合ドライバの扱い:

- 別ベンダドライバが interface を掴む環境では、そのドライバを削除する運用を前提にする。
- `rusb` の `claim_interface` が失敗した場合は、権限問題、デバイス未接続、別ドライバによる busy を区別して表示する。
- アプリ自身が別ドライバを停止、削除、置換する処理は初期実装に含めない。
- 公式 Mac DEXT との共存は目標にしない。

配布方針:

- 開発中は ad-hoc signing の `.app` でよい。
- 限定配布では、Gatekeeper の警告をユーザーが明示的に許可する手順を許容する。
- Developer ID 署名と notarization は、一般配布が必要になった時点で検討する。
- App Store 配布は目標にしない。

実装順:

1. `t6-virtual-display` の中核を library API へ寄せる。
2. Tauri backend から start / stop / status / rotate / reset を呼べるようにする。
3. メニューバー UI に接続状態、回転、開始停止、診断を置く。
4. device polling と再接続 state machine を入れる。
5. 自動起動、sleep / wake 復旧、ログ収集を追加する。

## 長期ロードマップ

### kext / DriverKit / System Extension

当面は kext、DriverKit、System Extension を使わない。
常駐化やデバイス検出は通常アプリで実装し、USB ownership は userspace `rusb` の interface claim に任せる。
別ベンダドライバとの競合は、ドライバ削除を前提にして扱う。

kext を採用しない理由:

- 現代 macOS では新規 kext の配布、許可、保守の負担が大きい。
- 今の構成では USB bulk/control/interrupt 転送が userspace からできている。
- kext 化しても `CGVirtualDisplay` の private API リスクは解消しない。

DriverKit を保留する理由:

- USB device を OS の display driver として登録する必要が現時点ではない。
- DriverKit 化すると entitlement、Developer ID、System Extension 許可、install / uninstall、IPC が増える。
- encode、dirty rect、type 7、rotation の未解決部分を先に安定させるほうが効果が大きい。

再検討する条件:

- 別ベンダドライバを削除しても `rusb` の interface claim が安定しない。
- sleep / wake や hotplug の復旧が通常アプリでは十分に安定しない。
- 一般配布で、ユーザーに競合ドライバ削除を求められない。
- USB ownership、権限、復旧性を OS 側の driver model に寄せる必要が出る。

その場合も、まず検討するのは DriverKit であり、kext ではない。
...
- Tauri メニューバーアプリ化の最小実装。

理由:

- path 自体は動いている。
- ただし 60 fps の安定性、回転、画質、latency spike が絡む。
- UI から既存 CLI 相当の開始、停止、回転、診断を呼ぶだけなら、driver model を変える必要はない。
...
- DriverKit は entitlement、install、IPC、競合ドライバ処理の実装面積が大きい。
- VRAM allocation と multi-output scheduling は、DriverKit とは別に解く必要がある。
...
## 2026-06-28 Type7 virtual display 実装メモ

2026-06-28 時点では、synthetic Type7 は保留から再開し、`t6-virtual-display` の実用経路としてかなり動くところまで進んだ。
ただし、まだ `--unsafe-generated-type7` 前提であり、長時間安定性は継続確認中である。

### 現状の結論

- `jpeg-type7` + `local` rect は、動画再生や通常操作でかなり有効。
- `horizontal-band` は小さい dirty でも横幅全体へ広がりやすく、動画領域では `tile-ratio` で full setup に逃げやすい。
- `local` では `path=type7` を維持しやすく、平均 total はおおむね 6-15 ms 程度まで下がる。
- 大きい tile では quality 95 のままだと encode/USB のピークが立つため、large tile だけ quality を落とすのが有効。
- Type7 command ring の wrap 直後に固まりやすい傾向がある。
- wrap 検出時は Type7 tile を送らず、`type4+type7` setup に逃がすと安定性が上がった。
- setup ack は取れないことが多く、`--type7-wait-setup-ack-ms 0` のほうが体感上よい。

### 現時点のデフォルト設定

2026-06-29 時点では、`t6-virtual-display` のデフォルトを次の実機 baseline に寄せている。

```sh
cargo run --release --features usb --bin t6-virtual-display -- \
    --width 1080 --height 1920 --rotate 90 \
    --fps 60 --quality 95 --subsamp 420 \
    --transport jpeg-type7 \
    --unsafe-generated-type7 \
    --type7-setup paired-type4-type7 \
    --type7-rect local \
    --type7-min-local-area-ratio 0 \
    --type7-max-tile-ratio 1.0 \
    --type7-refresh-frames 0 \
    --type7-wait-setup-ack-ms 0 \
    --type7-cmd-addr 0x03200000 \
    --type7-cmd-wrap-addr 0x03900000 \
    --wait-interrupt-ms 0 \
    --no-frame-throttle --drop-late-frames --type7-large-tile-fps-divisor 2 \
    --type7-jpeg-component-ids original --jpeg-encoder vt-full-turbo-tiles \
    --type7-large-tile-ratio 0.25 \
    --type7-large-tile-quality 88
```

補足:

- `--type7-cmd-wrap-addr 0x03a00000` も試したが、`0x039xxxxx` 付近まで使うと wrap 前後で固まることがあった。
- そのため、現時点では `0x03900000` 手前で wrap させるほうが安全そう。
- `--type7-jpeg-component-ids original` でも TurboJPEG / VideoToolbox ともに表示できた。
- component id patch は速度にはほぼ効かず、現時点の baseline では不要寄りとして扱う。
- `vt-full-turbo-tiles` は full frame / setup を VideoToolbox、Type7 tile を TurboJPEG で処理する。
- VideoToolbox を Type7 tile に直接使うと、初見 tile size で encoder 作成が刺さることがある。
- fullish 連発時はまだ重く、`--drop-late-frames` は遅延蓄積の緩和にとどまる。
- fullish を通常 full JPEG に逃がすと描画が崩れたため、fullish は Type4 setup 経路に残す。

### 実装に反映済みの点

- mode-6 Type7 の plane address は `plane0_addr/plane1_addr/plane2_addr` として扱う。
- `plane0_addr = base0 + top * canvas_width + left`
- `plane1_addr = base1 + (top / 2) * canvas_width + aligned_left`
- `plane2_addr = 0`
- canvas は `1920x1920` 相当、つまり `canvas_height = align16(output_width)` を使う。
- JPEG は baseline 4:2:0 を前提にする。
- component id は `original` のままでも表示できるため、現時点の baseline では書き換えない。
- Type4 setup と Type7 は同じ slot base を使う。
- command ring は `type7_jpeg_cmd_offset(jpeg_len)` で進める。
- ring wrap 検出時は `type7_full_reason=cmd-wrap` として full setup 側へ逃がす。
- 前フレームの dirty bbox と現在の dirty bbox を union し、カーソル残像を減らす。
- Type7 local tile では全画面 rotate 後 crop をやめ、tile だけ直接 crop/rotate する。
...
- `path=type7` が継続する。
- wrap 時だけ `path=type4+type7 type7_full_reason=cmd-wrap` が出る。
- `dropped=0 throttled=0 busy=0 late=0`
- large tile では `tile_q=88`
- 小-中サイズ tile では `tile_q=95`
- `interrupts=1 fences=1 matched=1 ack_lag=0` が多い。
- average total は 10 ms 前後、max total は数十 ms 程度に収まる。

### まだ残っている問題

- 稀に数秒級で固まることがある。
- 固まりは Type7 command ring の wrap 近辺、または VRAM 末尾付近の再利用と関係していそう。
- 実機 probe では VRAM size は 58 MB なので上限は `0x03a00000` で合っているはずだが、末尾近くが安全に使えるとは限らない。
- `type4+type7` setup に対する ack は取れないことが多い。
- 公式 driver が ring wrap 時にどのように同期・再初期化しているかは未解明。
- 全画面更新は依然として重い。現実的には large tile を quality 85-88、fps 30 相当に落とすのが妥当。

### 次に見ること

1. `--type7-cmd-wrap-addr 0x03900000` で長時間運用し、wrap 後の固まりが減るか確認する。
2. まだ固まる場合、wrap 後に数フレームだけ Type7 を止める cooldown を入れる。
3. large tile quality を `85` / `88` / `90` で比較し、信号機や動画の見た目とピーク負荷のバランスを見る。
4. Windows driver の Type7 ring allocator が wrap 時に setup/flush/interrupt wait を入れているか、Ghidra 側で再確認する。
5. `0x03900000` より低い wrap address も試し、安全な command ring 範囲を実測する。
